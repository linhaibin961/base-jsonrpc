...
import com.lhb.common.exception.BaseException;
import com.yzmy.jsonrpc4j.ErrorResolver;
...
public class ModelExceptionResolver implements ErrorResolver {
    ...
    public ModelExceptionResolver() {
    }

    @Override
    public JsonError resolveError(Throwable t, Method method, List<JsonNode> arguments) {
        log.error("用户端接口调用错误", t);
        ...
        /*if ( t instanceof DataAccessException){ // SQL异常
            return new JsonError(-31996,t.getMessage(),t.getStackTrace());
        }

        if (t instanceof MobileAlreadyRegisteredException) { // 处理发送验证码时手机已经注册异常
            return new JsonError(ErrorCode.MVCODE_MOBILE_ALREADY_EXISTS.getCode(), t.getMessage(), t.getStackTrace());
        }
*/
